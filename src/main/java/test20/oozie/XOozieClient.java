...
public class XOozieClient extends OozieClient {
    ...
    private void validateHttpSubmitConf(Properties conf) {
        String JT = conf.getProperty(org.apache.oozie.client.XOozieClient.JT);
        String JT_2 = conf.getProperty(org.apache.oozie.client.XOozieClient.JT_2);
        if (JT == null) {
            if(JT_2 == null) {
                throw new RuntimeException("jobtracker is not specified in conf");      //08.抛运行时异常,为什么不是
            }
        }

        String NN = conf.getProperty(org.apache.oozie.client.XOozieClient.NN);
        String NN_2 = conf.getProperty(org.apache.oozie.client.XOozieClient.NN_2);
        if (NN == null) {
            if(NN_2 == null) {
        ...
        if (!libPath.contains(":/")) {
            ...
            conf.setProperty(LIBPATH, newLibPath);              //改完再设回去
        }
    ...
    public String submitScriptLanguage(Properties conf, String scriptFile, String[] args, String[] params, String jobType)
        ...
        if (jobType.equals(OozieCLI.HIVE_CMD)) {
            script = org.apache.oozie.client.XOozieClient.HIVE_SCRIPT;
            options = org.apache.oozie.client.XOozieClient.HIVE_OPTIONS;
            scriptParams = org.apache.oozie.client.XOozieClient.HIVE_SCRIPT_PARAMS;
        }
        else if (jobType.equals(OozieCLI.PIG_CMD)) {
            script =  org.apache.oozie.client.XOozieClient.PIG_SCRIPT;
            options = org.apache.oozie.client.XOozieClient.PIG_OPTIONS;
            scriptParams = org.apache.oozie.client.XOozieClient.PIG_SCRIPT_PARAMS;
        }
        else {
     ...
     * @throws org.apache.oozie.client.OozieClientException thrown if the job could not be submitted.
     */
    public String submitSqoop(Properties conf, String[] command, String[] args)     //06. 三个参数
            throws OozieClientException {
        if (conf == null) {
            throw new IllegalArgumentException("conf cannot be null");
        }
        if (command == null) {
            throw new IllegalArgumentException("command cannot be null");           //如果为空,抛IllegalArgumentException
        }

        validateHttpSubmitConf(conf);       //07. 验证

        conf.setProperty(org.apache.oozie.client.XOozieClient.SQOOP_COMMAND, serializeSqoopCommand(command));       //String[] -> String
        setStrings(conf, org.apache.oozie.client.XOozieClient.SQOOP_OPTIONS, args);

        return (new HttpJobSubmit(conf, OozieCLI.SQOOP_CMD)).call();            //08. call返回一个jobid,调call的在父类创建了Conn,父类调了子类的call(HttpURLConnection conn)
    }
    ...
    private class HttpJobSubmit extends ClientCallable<String> {        //09.ClientCallable的子类,传入<String>泛型
        private Properties conf;

        HttpJobSubmit(Properties conf, String jobType) {
            super("POST", RestConstants.JOBS, "", prepareParams(RestConstants.JOBTYPE_PARAM, jobType));     //父类构造
